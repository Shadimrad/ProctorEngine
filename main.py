    ...
    while True:
        ...
        if res.multi_face_landmarks:
            ...
            if okhp:
                ...
                if hit:
                    u,v,_=hit; px,py=int(u*w),int(v*h)
...
def main():
    ...
    cal=sub.add_parser('calibrate'); cal.add_argument('--out',required=True); cal.add_argument('--screen-w',type=float,required=True); cal.add_argument('--screen-h',type=float,required=True); cal.add_argument('--cam',type=int,default=0)
    tr=sub.add_parser('track'); tr.add_argument('--plane',required=True); tr.add_argument('--cam',type=int,default=0)
    args=ap.parse_args()
    ...
    if args.cmd=='calibrate':
        ...
        plane=ScreenPlane(data['w'],data['h']); plane.O=np.array(data['O']); plane.Ux=np.array(data['Ux']); plane.Uy=np.array(data['Uy']); plane.n=np.array(data['n'])
        live_loop(cap,plane)
